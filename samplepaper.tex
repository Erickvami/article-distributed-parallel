...
On this architecture it is not counted the number of iterations by a synchronous way, then we don't know 
if the algorithm is just interating more than usual and taking more computational resources with a real stop criterial.
To solve the mencioned problematic there is used a method that defines when to stop the algorithms using as a metric value 
the number of evaluations. Using a maximum number of evaluations it could be defined an acceptable criterial to stop, gived by the next ecuation.
