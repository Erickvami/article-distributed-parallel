...
monolithic to serverless architectures, in this case we are using AWS functions, 
as seen in Table~\ref{table:architectures}.     
...
We propose is an architecture that allows the processing of multi-populations
...
This component receives the configuration parameters of
 ...
 Once a subpopulation is processed, the manager pulls
  ...
  Uniform crossing, which will be explained below, between the two. 
  As a result of the migration, we have now created two new
...
parameters, and returning the evolved population. % Is this why you
...
% But you need to describe some kind of migration selection - JJ
...
%Initial experiments using this architecture proved that it is completely
%scalable and useful for the implementation of
%multiple algorithms. % You have not proved anything about scalability
                     % - JJ
...
achieved % comment something on the overall time needed for each
% experiment. - JJ
